...
GradientPair ClassicGradient::operator()(const VectorXd& x, const MatrixXd& u, double learning_rate) {
...
MomentumGradient::MomentumGradient(size_t x_size, size_t u_size, double alpha)
    : momentum_A_(MatrixXd::Zero(u_size, x_size)), momentum_b_(VectorXd::Zero(u_size)), alpha_(alpha) {
}

GradientPair MomentumGradient::operator()(const VectorXd& x, const MatrixXd& u, double learning_rate) {
...
AdaGradient::AdaGradient(size_t x_size, size_t u_size)
    : A_g_(MatrixXd::Zero(u_size, x_size)), b_g_(VectorXd::Zero(u_size)) {
}

GradientPair AdaGradient::operator()(const VectorXd& x, const MatrixXd& u, double learning_rate) {
    MatrixXd A_grad(u.transpose() * x.transpose());
    VectorXd b_grad(u.transpose());
...
RMSPropGradient::RMSPropGradient(size_t x_size, size_t u_size, double alpha)
    : A_g_(MatrixXd::Zero(u_size, x_size)), b_g_(VectorXd::Zero(u_size)), alpha_(alpha) {
}

GradientPair RMSPropGradient::operator()(const VectorXd& x, const MatrixXd& u, double learning_rate) {
    MatrixXd A_grad(u.transpose() * x.transpose());
    VectorXd b_grad(u.transpose());
...
AdamGradient::AdamGradient(size_t x_size, size_t u_size, double alpha_linear, double alpha_square)
    : A_linear_(MatrixXd::Zero(u_size, x_size)), A_square_(MatrixXd::Zero(u_size, x_size)),
      b_linear_(VectorXd::Zero(u_size)), b_square_(VectorXd::Zero(u_size)),
      alpha_linear_(alpha_linear), alpha_square_(alpha_square),
      alpha_linear_step_(1.0), alpha_square_step_(1.0) {
}

GradientPair AdamGradient::operator()(const VectorXd& x, const MatrixXd& u, double learning_rate) {
    MatrixXd A_grad(u.transpose() * x.transpose());
    VectorXd b_grad(u.transpose());
    ...
    MatrixXd A_m(((A_square_.array() + epsilon) / (1.0 - alpha_square_step_)).sqrt().inverse().matrix());
    MatrixXd b_m(((b_square_.array() + epsilon) / (1.0 - alpha_square_step_)).sqrt().inverse().matrix());
